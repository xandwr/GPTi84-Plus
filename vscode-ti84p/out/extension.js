...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor((editor) => {
        if (editor)
            runLinter(editor.document, collection);
    ...
    // Completion provider
    const completionProvider = vscode.languages.registerCompletionItemProvider({ language: "ti84p" }, {
...
// Helpers
// ---------------------------------------------------------------------------
function runLinter(document, collection) {
    if (document.languageId !== "ti84p")
        return;
    const diags = (0, linter_1.lint)(document.getText());
    collection.set(document.uri, diags.map((d) => {
        const range = new vscode.Range(new vscode.Position(d.line, d.col), new vscode.Position(d.line, d.endCol));
