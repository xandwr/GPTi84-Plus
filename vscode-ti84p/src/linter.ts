...
export interface Diagnostic {
  line: number;   // 0-based
  col: number;    // 0-based, start of offending token
  endCol: number;
  message: string;
...
const CE_CMD_RE = new RegExp(
  `\\b(${[...CE_ONLY_COMMANDS].map((c) => c.replace(/[()]/g, "\\$&")).join("|")})\\b`,
  "g"
);
...
interface StackFrame {
  opener: string;
  line: number;
  hasIf: boolean; // true if this Then follows an If
}
      ...
      if (inLoop) {
        ...
        if (gotoMatch) {
          diagnostics.push({
            ...
            message:
              "`Goto` out of a loop skips `End` and leaks the loop's stack frame, " +
